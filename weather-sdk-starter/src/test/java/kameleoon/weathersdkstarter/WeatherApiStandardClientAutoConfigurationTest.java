...
@TestPropertySource(properties = {"openweather.client.reactive=false"})
class WeatherApiStandardClientAutoConfigurationTest {
    ...
    @Test
    void shouldCreateStandardClientOnly() {
        assertThat(standardClient)
                .as("Standard client must be created when reactive=false")
                .isNotNull();
        assertThat(reactiveClient)
                .as("Reactive client must NOT be created when reactive=false")
                .isNull();
    }
